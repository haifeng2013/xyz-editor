...
export const geoToGrid = (longitude: number, latitude: number, level: number): Grid => {
    const zoomScale = MATH.pow(2, level);

    const row = MATH.floor(projection.lon2x(longitude, zoomScale));
    const col = MATH.floor(projection.lat2y(latitude, zoomScale));

    return [level, col, row];
};

export const pixelToGrid = (x: number, y: number, level: number): Grid => {
...
export const getTilesIds = (topLeft: Grid, bottomRight: Grid): Quadkeys => {
    ...
    const cols = bottomRight[2] - topLeft[2] + 1;

    for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
            tiles[tiles.length] = tileXYToQuadKey(topLeft[0], topLeft[1] + y, topLeft[2] + x);
        }
    }
