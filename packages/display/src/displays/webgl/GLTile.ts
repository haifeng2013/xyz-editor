...
class GLTile extends BasicTile {
    ...
    constructor(quadkey: string, layers: any[], size: number, onDrop?: (data: any[]) => void) {
        super();
        this.onDrop = onDrop;
        this.init(quadkey, layers, size);
    }
    ...
    init(quadkey: string, layers: TileLayer[], size: number) {
        super.init(quadkey, layers, size);
