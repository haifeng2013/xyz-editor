...
import {GeometryGroup} from './buffer/GeometryGroup';
...
export class GLRender implements BasicRender {
    ...
    constructor(renderOptions: RenderOptions) {
        this.ctxAttr = {
            alpha: true, // true -> leads to aliasing on lines!?
            antialias: false,
            depth: true,
            stencil: true,
    ...
    init(canvas: HTMLCanvasElement, devicePixelRation: number): void {
        ...
        // this.glyphs = new GlyphManager(gl, texUnits - 1);
    ...
    initView(pixelWidth: number, pixelHeight: number, scale: number, rotX: number, rotZ: number) {
        ...
        const centerPixelX = pixelWidth / 2;
        const centerPixelY = pixelHeight / 2;
        // plane where one texel equals one pixel
        const targetZ = pixelHeight / (2 * Math.tan(VFOV / 2));
        // const zNear = 650
        // const zFar = 1550;
        const zNear = targetZ / 5; // * 0.83;
        const zFar = targetZ * 1.5; // * 1.25
        ...
        // console.log(pixelWidth,'x',pixelHeight,'targetZ', targetZ, 'zNear', zNear, 'zFar', zFar, 'scale',scale );

        // scale = scale*0.5;

        mat4.perspective(projectionMatrix, VFOV, pixelWidth / pixelHeight, zNear, zFar);
    ...
    deleteBuffer(buffer: GeometryBuffer) {
        ...
        for (let name in attributes) {
            ...
            let glBuffer = this.buffers.get(attr);
    ...
    private grpFilter: (group: GeometryGroup) => boolean;

    setGroupFilter(filter?: (group: GeometryGroup) => boolean) {
    ...
    private drawBuffer(buffer: GeometryBuffer, x: number, y: number, pMat?: Float32Array, dZoom?: number/* , customGLStates?*/): void {
        ...
        // const skipType = this.skipType;
        for (let g = 0; g < groups.length; g++) {
            group = groups[g];
            program = this.programs[group.type];

            if (this.grpFilter && !this.grpFilter(group)) {
                continue;
            }
            ...
            if (program) {
                ...
                if (pass) {
                    ...
                    program.initAttributes(bufAttributes, buffers/* .get(bufAttributes)*/);

                    groupAttributes = group.getAttributes();

                    this.initBuffers(groupAttributes);

                    program.initAttributes(groupAttributes, buffers/* .get(groupAttributes)*/);

                    program.initUniforms(group.uniforms);

                    uLocation = program.uniforms;

                    gl.uniform1f(uLocation.u_rotate, this.rz);
                    gl.uniform2f(uLocation.u_resolution, this.w, this.h);
                    gl.uniform1f(uLocation.u_zIndex, -0.0013 * (renderLayer.getZ(group.zIndex) + 100 * renderLayer.index));
                    ...
                    program.draw(group, buffers, this.pass);
                }
            } else {
