...
abstract class Display {
    ...
    initGrid(rotatedScreenPixels, zoomLevel: number, gridTileSize: number, screenOffsetX: number, screenOffsetY: number) {
        const display = this;
        const layers: Layer[] = <Layer[]><unknown> this.layers;
        const gridView = display.grid.getGrid(zoomLevel - Number(gridTileSize == 512), 1);
        const gridTiles = gridView.tiles;
        const gridOffsetX = gridView.x * gridTileSize - screenOffsetX;
        const gridOffsetY = gridView.y * gridTileSize - screenOffsetY;
        const prevVPTiles = display.tiles[gridTileSize];

        let vpTiles = display.tiles[gridTileSize] = [];
        let tileGeoContainer: [number, number][] = [[0, 0], [0, 0], [0, 0], [0, 0]];
        const screenTiles = [];

        for (let row = 0, rows = gridTiles.length; row < rows; row++) {
            for (let col = 0, cols = gridTiles[row].length; col < cols; col++) {
                let x1 = col * gridTileSize + gridOffsetX;
                let y1 = row * gridTileSize + gridOffsetY;
                let x2 = x1 + gridTileSize;
                let y2 = y1 + gridTileSize;
                let quadkey = gridTiles[row][col];

                x1 = Math.round(x1);
                y1 = Math.round(y1);

                tileGeoContainer[0][0] = x1;
                tileGeoContainer[0][1] = y1;

                tileGeoContainer[1][0] = x2;
                tileGeoContainer[1][1] = y1;

                tileGeoContainer[2][0] = x2;
                tileGeoContainer[2][1] = y2;

                tileGeoContainer[3][0] = x1;
                tileGeoContainer[3][1] = y2;

                if (doPolygonsIntersect(rotatedScreenPixels, tileGeoContainer)) {
    ...
    updateGrid(viewport: GeoRect, zoomLevel: number, screenOffsetX: number, screenOffsetY: number) {
        const display = this;
        ...
        grid.init(viewport, rotZRad, mapWidthPixel, mapHeightPixel);
        ...
        for (let tileSize of tileSizes) {
            this.initGrid(rotatedScreenPixels, zoomLevel, tileSize, screenOffsetX, screenOffsetY);
        }
        this.dirty = true;
