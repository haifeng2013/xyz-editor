...
class TigerMap {
    ...
    private _s: number; // current scale
    ...
    private initViewPort(): geo.Rect {
        ...
        return this._vp = new GeoRect(topLeftLon, bottomRightLat, bottomRightLon, topLeftLat);
    };
    ...
    pitch(x) {
        if (x !== UNDEF) {
            ...
            // increase grid size to fully fill screen if rotated by x axis.
            // unneded/invisible tiles will be "cut" later on
            // scaleGrid(1 - xdeg * 2.5 / 100);
            this.updateGrid();
        }
        return -this._rx * 180 / Math.PI;
    };

    rotate(deg?: number, rx?: number, ry?: number) {
        if (deg !== UNDEF) {
            let rad = (deg | 0) * Math.PI / 180;
            ...
            if (rad !== rotZRad) {
                ...
                let deltaRotationCenterX = this._cx - rx;
                let deltaRotationCenterY = this._cy - ry;

                // apply current set rotation
                let rDelta = rotate(deltaRotationCenterX, deltaRotationCenterY, 0, 0, -rotZRad);
                let prevRot = rotZRad;

                let centerWorldPixel = this._cw;

                // compensate current set scale
                centerWorldPixel.x -= rDelta[0] / currentScale;
                centerWorldPixel.y -= rDelta[1] / currentScale;

                this._rz = rad;

                // pan is already taking care of rotation
                this.pan(-deltaRotationCenterX, -deltaRotationCenterY);

                this.updateGrid();

                this._l.trigger('rotation',
                    ['rotation', this._rz, prevRot],
                    true
                );
            }
        }
    ...
    setZoomlevel(zoomTo: number, fixedX?: number, fixedY?: number, animate?: number) {
        ...
        const screenToWorld = (absScreen, scale) => {
            absScreen[0] = this._tlwx + absScreen[0] / scale;
            absScreen[1] = this._tlwy + absScreen[1] / scale;
            // toAbsoluteScreenPixels(absScreen);
            // absScreen[0] = topLeftX + absScreen[0];
            // absScreen[1] = topLeftY + absScreen[1];
            return absScreen;
        };
        ...
        if (animate) {
            ...
            let zFromFloat = zoomLevel + (currentScale % 1);
            // let zFromFloat = tigerMap.getZoomlevel();
            let deltaLevel = (zoomTo ^ 0) - zoomLevel;
            let scale = 1 + Math.round((zoomTo % 1) * 1e4) / 1e4;

            const startScale = this._s;

            // setScale(scale, fixedX, fixedY);
            // take current rotation into account for unrotated incoming screencoordinates
            let coord = rotate(fixedX, fixedY, this._cx, this._cy, -this._rz);
            let deltaDisplayCenterX = this._w / 2 - coord[0];
            let deltaDisplayCenterY = this._h / 2 - coord[1];

            screenToWorld(coord, this._s);
            const worldSizePixel = this._wSize;

            this._setCenter(
                project.x2lon(coord[0] + deltaDisplayCenterX / scale, worldSizePixel),
                project.y2lat(coord[1] + deltaDisplayCenterY / scale, worldSizePixel)
            );
            ...
            if (deltaLevel) {
                ...
                let scaleCenterPixel = rotate(fixedX || 0, fixedY || 0, this._cx, this._cy, -this._rz);
                // take current scaling into account
                // scaleCenterPixel = toAbsoluteScreenPixels( scaleCenterPixel.x, scaleCenterPixel.y );
                let zoomDelta = this._z - zoomTo;
                let deltaZoomCompensator = Math.pow(.5, zoomDelta);


                if (zoomDelta !== 0) {
                    screenToWorld(scaleCenterPixel, startScale);

                    let zoomFromWorlSizePixel = this._wSize;
                    ...
                    let scaleCenterLon = project.x2lon(
                        scaleCenterPixel[0] + ((centerWorldPixel.x - scaleCenterPixel[0]) / (deltaZoomCompensator)),
                        zoomFromWorlSizePixel
                    );
                    let scaleCenterLat = project.y2lat(
                        scaleCenterPixel[1] + ((centerWorldPixel.y - scaleCenterPixel[1]) / (deltaZoomCompensator)),
                        zoomFromWorlSizePixel
                    );

                    // set new center to fixed scale postition
                    this.setCenter(scaleCenterLon, scaleCenterLat);
                }
            } else {
                this.updateGrid();
            }
    ...
    pan(dx: number, dy: number, _dx?: number, _dy?: number) {
        ...
        dx = (dx ^ 0 - (_dx ^ 0)) / currentScale;
        dy = (dy ^ 0 - (_dy ^ 0)) / currentScale;

        // make sure dx or dy results in real viewport change..
        // otherwise viewport would not get cleared and rerendering can be blurry
        if (dx != 0 || dy != 0) {
            let rDelta = rotate(dx, dy, 0, 0, -this._rz);

            let centerWorldPixel = this._cw;

            let x = centerWorldPixel.x - rDelta[0];
            let y = centerWorldPixel.y - rDelta[1];
    ...
    pixelToGeo(x: number | pixel.Point, y?: number) {
        ...
        // converts screenpixel to projected screenpixels
        let p = this._display.unproject(x, y, screenOffsetX, screenOffsetY);
