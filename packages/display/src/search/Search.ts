...
export class Search {
    ...
    private getFeaturesInRect(x1: number, y1: number, x2: number, y2: number, layers, zoomlevel: number, mostTopFeatureOnly?: boolean) {
        const {map, hit} = this;
        let x = x1 + (x2 - x1) / 2;
        let y = y1 + (y2 - y1) / 2;
        let minLon = 180;
    ...
    search(x: number, y: number, x2: number, y2: number, layers, mostTopFeatureOnly?: boolean) {
        const {map} = this;
