...
export const eObservers = (observerHandler) => {
    return {
         ...
         *      The context in which the given function should be called (default null); if omitted, the global object is used
         *
         *  @name here.xyz.maps.editor.Editor#addObserver
         */
        addObserver: (key, observer, context) => {
        ...
        removeObserver: (key, observer) => {
            observerHandler.removeObserver(key, observer);
        }
    };
};
